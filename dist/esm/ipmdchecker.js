...
export class IpmdChecker {
    ...
    loadTestEtJson() {
        if (!fs.existsSync(this._testEtJsonFp))
            return {};
        let stringdata = fs.readFileSync(this._testEtJsonFp, { "encoding": "utf-8" });
        let testdata;
        try {
    ...
    compareIpmdCheckerResults(resultRef, resultTest, ipmdIdFilter, compareOptions) {
        ...
        let refDataValueFsd = new FixedStructureData(refDataValue, false);
        let testDataValue = resultTest.value;
        let testDataValueFsd = new FixedStructureData(testDataValue, false);
