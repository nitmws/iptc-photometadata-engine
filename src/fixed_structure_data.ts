...
class FixedStructureData {
   ...
   */
  constructor(fsData: object, readAndWrite = true) {
  ...
  public getFsData(
    ...
    const selectors: string[] = selectpath.split(levelsep);
    const startResult: object = { state: "SEARCHING" };
    const returnObj: object = this._digGetFsData(
      1,
      selectors,
      indexsep,
      this._fsData,
        ...
        if (selAddressesArray) {
          if (Array.isArray(digFsData[selector])) {
            if (digFsData[selector].length > selectorIdx) {
              const selectedDigFsData: object =
                digFsData[selector][selectorIdx];
              if (selectors.length === 0) {
                thisResult["state"] = "FOUND";
                thisResult["value"] = selectedDigFsData;
                return thisResult;
              } else {
                thisResult = this._digGetFsData(
                  level + 1,
                  selectors,
                  indexsep,
                  selectedDigFsData,
    ...
    if (selectors.length > 0) {
      ...
      if (activeDigFsData.hasOwnProperty(selector)) {
        if (selectors.length === 0) {
          ...
          thisResult = this._digSetFsData(
            level + 1,
            selectors,
            indexsep,
            activeDigFsData[selector],
            thisResult,
            setvalue
          );
          return thisResult;
