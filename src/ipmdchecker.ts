...
export class  CompareResultRow {
    result: string = '';  // see icc.cmpgRC...
    message: string = '';
    comparedNamePath: string = '';
    comparedIpmdIdPath: string = '';
    comparedValueFormat: string = '';  // XMP or IIM
    refValue: string = '';
    testValue: string = '';
    dispparam1: string = '';  // parameters for displaying the results, can be used outside the IPTC PMD Checker
    dispparam2: string = '';
    dispparam3: string = '';
}
...
export class IpmdChecker {
    ...
    public getIpmdTopPropertyData(propertyId: string): object {
        ...
        let iptcTop = this.ipmdRef["ipmd_top"];
    ...
    public getIpmdStructData(structId: string): object {
        ...
        let iptcStruct = this.ipmdRef["ipmd_struct"];
    ...
    public loadTestEtJson (): object {
        if (!fs.existsSync(this._testEtJsonFp))
            return {};
        let stringdata: string = fs.readFileSync(this._testEtJsonFp, {"encoding": "utf-8"});
        let testdata: object[];
        try {
    ...
    public checkIpmdStd(
        ...
        compareValues: boolean = false,
        countOccurrences: boolean = false,
        anyOtherDataRef: MdStruct = {}): IipmdCheckerResult{
        // prerequisites
        ...
        for (const refPropId in refIpmdTop) {
            ...
            let iimValue: any;
            let exifValue: any;
            let propVresult: MdStruct = {};
            let refPropData = refIpmdTop[refPropId];
            let etXmpId = refPropData[icc.itgEtXmp];
            if (testImgEtPmd.hasOwnProperty(etXmpId)) {
                this._ipmdStateData.setFsData(1,refPropId + this._lsep + icc.stateData + this._lsep +
                icc.stateDxmp);
                let datatype: string = refPropData[icc.itgDatatype];
                if (datatype === icc.itgDtStruct){
                    let structureId = refPropData[icc.itgDataformat];
                    if (structureId !== icc.itgDfAlg){
                        let structureTestValue = testImgEtPmd[etXmpId];
                        let checkStructVresult: object[] = this._checkIpmdStdStruct(refPropId,
                            structureId, structureTestValue, countOccurrences);
                        if (!util1.objectIsEmpty(checkStructVresult)){
                            ...
                            if (countOccurrences){
                                if (Array.isArray(checkStructVresult)){
                                    this._ipmdStateData.setFsData(checkStructVresult.length,
                                        refPropId + this._lsep + icc.stateData + this._lsep +
                                        icc.stateDvaloccur);
                                }
                                else {
                                    this._ipmdStateData.setFsData(1,
                                        refPropId + this._lsep + icc.stateData + this._lsep +
                                        icc.stateDvaloccur);
                                }
                            }
                    ...
                    else { // AltLang property
                        xmpValue = this._buildAltLangValue(testImgEtPmd, etXmpId);
                        propVresult[icc.stateVxmp] = xmpValue;
                        if (countOccurrences){
                            if (Array.isArray(xmpValue)){
                                this._ipmdStateData.setFsData(xmpValue.length,refPropId + this._lsep + icc.stateData + this._lsep +
                                    icc.stateDvaloccur);
                            }
                            else {
                                this._ipmdStateData.setFsData(1,refPropId + this._lsep + icc.stateData + this._lsep + icc.stateDvaloccur);
                            }
                        }
                    }
                }
                else {
                    propVresult[icc.stateVxmp] = testImgEtPmd[etXmpId];
            ...
            if (refPropData.hasOwnProperty(icc.itgEtIim)){
                let etIimId = refPropData[icc.itgEtIim];
                if (etIimId === "IPTC:DateCreated+IPTC:TimeCreated"){
                    if (testImgEtPmd.hasOwnProperty("IPTC:DateCreated") &&
                        ...
                        this._ipmdStateData.setFsData(1,refPropId + this._lsep + icc.stateData + this._lsep +
                            icc.stateDiim);
                        propVresult[icc.stateViim] = testImgEtPmd["IPTC:DateCreated"] + " " + testImgEtPmd["IPTC:TimeCreated"];
                        iimValue = testImgEtPmd["IPTC:DateCreated"] + " " + testImgEtPmd["IPTC:TimeCreated"];
                    }
                }
                if (testImgEtPmd.hasOwnProperty(etIimId)) {
                    ...
                    if (etIimId === 'IPTC:By-line'){
                        let tempIimValue: string[] = [];
                        tempIimValue.push(testImgEtPmd[etIimId]);
                        propVresult[icc.stateViim] = tempIimValue;
                        iimValue = tempIimValue;
                    }
                    else {
                        propVresult[icc.stateViim] = testImgEtPmd[etIimId];
            ...
            if (refPropData.hasOwnProperty(icc.itgEtExif)){
                let etExifId = refPropData[icc.itgEtExif];
                let exifDataSet: boolean = false;  // changed to true if any Exif data value is set
                ...
                if (etExifId === "ExifIFD:DateTimeOriginal+ExifIFD:TimeZoneOffset"){
                    if (testImgEtPmd.hasOwnProperty("ExifIFD:DateTimeOriginal")) {
                        this._ipmdStateData.setFsData(1,refPropId + this._lsep + icc.stateData + this._lsep +
                            icc.stateDexif);
                        propVresult[icc.stateVexif] = testImgEtPmd["ExifIFD:DateTimeOriginal"];
                        exifValue = testImgEtPmd["ExifIFD:DateTimeOriginal"];
                        exifDataSet = true;
                    }
                }
                if (etExifId === "ExifIFD:DateTimeOriginal+ExifIFD:SubSecTimeOriginal+ExifIFD:TimeZoneOffset"){
                    if (testImgEtPmd.hasOwnProperty("ExifIFD:DateTimeOriginal")) {
                        this._ipmdStateData.setFsData(1,refPropId + this._lsep + icc.stateData + this._lsep +
                            icc.stateDexif);
                        propVresult[icc.stateVexif] = testImgEtPmd["ExifIFD:DateTimeOriginal"];
                ...
                if (etExifId === 'IFD0:ImageDescription|ExifIFD:UserComment'){
                    if (testImgEtPmd.hasOwnProperty("IFD0:ImageDescription")) {  // try ImageDescription first ...
                        this._ipmdStateData.setFsData(1,refPropId + this._lsep
                            + icc.stateData + this._lsep + icc.stateDexif);
                        propVresult[icc.stateVexif] = testImgEtPmd["IFD0:ImageDescription"];
                        exifValue = testImgEtPmd["IFD0:ImageDescription"];
                        exifDataSet = true;
                    }
                    else {
                        if (testImgEtPmd.hasOwnProperty("ExifIFD:UserComment")) {  //...alternative: try UserComment
                            this._ipmdStateData.setFsData(1,refPropId + this._lsep
                                + icc.stateData + this._lsep + icc.stateDexif);
                            propVresult[icc.stateVexif] = testImgEtPmd["ExifIFD:UserComment"];
                ...
                if (etExifId === 'IFD0:ImageDescription' && refPropId === 'title') {
                    if (testImgEtPmd.hasOwnProperty(etExifId)) {
                        let testValue: string = testImgEtPmd[etExifId];
                        if (!testValue.includes(' ')) {
                            this._ipmdStateData.setFsData(1, refPropId + this._lsep
                                + icc.stateData + this._lsep + icc.stateDexif);
                            propVresult[icc.stateVexif] = testValue;
                            exifValue = testValue;
                            exifDataSet = true;
                        } else {
                ...
                if (etExifId === 'IFD0:Artist'){
                    if (testImgEtPmd.hasOwnProperty(etExifId)) {
                        let tempExifValue: string[] = [];
                        tempExifValue.push(testImgEtPmd[etExifId]);
                        propVresult[icc.stateVexif] = tempExifValue;
                ...
                if (!exifDataSet && testImgEtPmd.hasOwnProperty(etExifId)) {
                    this._ipmdStateData.setFsData(1,refPropId + this._lsep + icc.stateData + this._lsep +
                        icc.stateDexif);
                    propVresult[icc.stateVexif] = testImgEtPmd[etExifId];
            ...
            if (compareValues){
                if (xmpValue !== undefined && iimValue !== undefined){
                    let xmpIimAreEqual: boolean = false;
                    if (Array.isArray(xmpValue) && Array.isArray(iimValue)){
                        switch (refPropId){
                            case 'subjectCodes':
                                let valequ: boolean = true;
                                for (let idx = 0; idx < xmpValue.length; idx++){
                                    let scode = xmpValue[idx];
                                    let iimtestval = 'IPTC:' + scode;
                    ...
                    if (xmpIimAreEqual){
                        this._ipmdStateData.setFsData(1,refPropId + this._lsep
                            + icc.stateData + this._lsep + icc.stateDinsync);
                        if (exifValue !== undefined){
                            let iptcExifAreEqual: boolean = false;
                            ...
                            if (iptcExifAreEqual){
                                this._ipmdStateData.setFsData(1,refPropId + this._lsep
                                    + icc.stateData + this._lsep + icc.stateDmapinsync);
                            }
                            else {
                                this._ipmdStateData.setFsData(0,refPropId + this._lsep
                                    + icc.stateData + this._lsep + icc.stateDmapinsync);
                            }
                        }
                    }
        ...
        for (const refPropId in anyOtherDataRef){
            ...
            if (etTag !== ''){
                if (testImgEtPmd.hasOwnProperty(etTag)){
                    let propVresult: MdStruct = {};
                    propVresult[icc.stateVet] = testImgEtPmd[etTag];
                    this._ipmdValueData[icc.stateVaodPrefix + etTag] = propVresult;
                }
            }
        }
        // create the check result object
        let checkResult: IipmdCheckerResult = { state: {}, value: {}};
    ...
    private _checkIpmdStdStruct(
        ...
        countOccurrences: boolean = false ): object[] {
        ...
        let parentIpmdIds: string[] = parentIpmdIdsStr.split("/");
        // parentIpmdIds = parentIpmdIds.reverse();
        let parentStatePath: string = "";
        ...
        let structVresultArr: MdStruct[] = [];
        teststructEtPmdArr.forEach(teststructEtPmdOfArr => {
            let structVresult: MdStruct = {};
            let refIpmdStruct = this.ipmdRef[icc.itgIpmdStruct][refstructId];
            ...
            for (const refPropId in refIpmdStruct) {
                ...
                let etTag = refPropData[icc.itgEtTag];
                if (teststructEtPmdOfArr.hasOwnProperty(etTag)) {
                    this._ipmdStateData.setFsData(1, parentStatePath +
                        refPropId + this._lsep + icc.stateData + this._lsep + icc.stateDxmp);
                    let datatype: string = refPropData[icc.itgDatatype];
                    if (datatype === icc.itgDtStruct) {
                        let structureId = refPropData[icc.itgDataformat];
                        if (structureId !== icc.itgDfAlg) {
                            let structureTestValue = teststructEtPmdOfArr[etTag];
                            let parentIpmdIdsStrSub = parentIpmdIdsStr + "/" + refPropId;
                            let checkStructVresult: object[] = this._checkIpmdStdStruct(parentIpmdIdsStrSub,
                                structureId, structureTestValue, countOccurrences);
                            if (!util1.objectIsEmpty(checkStructVresult)) {
                                propVresult[icc.itgDtStruct] = checkStructVresult;
                                propVresult[icc.itgDtStruct] = checkStructVresult;
                                if (countOccurrences){
                                    if (Array.isArray(checkStructVresult)){
                                        this._ipmdStateData.setFsData(checkStructVresult.length,
                                            parentStatePath + refPropId + this._lsep + icc.stateData + this._lsep +
                                            icc.stateDvaloccur);
                                    }
                                    else {
                                        this._ipmdStateData.setFsData(1,
                            ...
                            propVresult[icc.stateVxmp] = this._buildAltLangValue(teststructEtPmdOfArr, etTag);
                            if (countOccurrences){
                                if (Array.isArray(teststructEtPmdOfArr[etTag])){
                                    this._ipmdStateData.setFsData(teststructEtPmdOfArr[etTag].length,
                                        parentStatePath + refPropId + this._lsep + icc.stateData + this._lsep +
                                        icc.stateDvaloccur);
                                }
                                else {
                                    this._ipmdStateData.setFsData(1,parentStatePath + refPropId +
                                        this._lsep + icc.stateData + this._lsep + icc.stateDvaloccur);
                                }
                            }
                        }
                    } else {
                        propVresult[icc.stateVxmp] = teststructEtPmdOfArr[etTag];
                        if (countOccurrences){
                            if (Array.isArray(teststructEtPmdOfArr[etTag])){
                                this._ipmdStateData.setFsData(teststructEtPmdOfArr[etTag].length,
                                    parentStatePath + refPropId + this._lsep + icc.stateData + this._lsep +
    ...
    public compareIpmdCheckerResults(
        ...
        ipmdIdFilter: string[],
        compareOptions: CompareOptions): CompareResultRow[] {
        let refDataValue: MdStruct = resultRef.value;

        let refValueIpmdIds: string[] = [];
        let refValueIpmdIdsPre: string[] = Object.keys(refDataValue);
        ...
        let refDataValueFsd: FixedStructureData = new FixedStructureData(refDataValue, false);

        let testDataValue: MdStruct = resultTest.value;
        let testDataValueFsd: FixedStructureData = new FixedStructureData(testDataValue, false);
        let compareResultRows: CompareResultRow[] = []; // this will be retured as result of this method
        // interate across the top level properties in the values of the reference file
        for (let idx = 0; idx < refValueIpmdIds.length; idx++){
            let refIpmdId: string = refValueIpmdIds[idx];
            // set data from the formal IPMD reference data
            let shouldValFmts: XmpIimTwins = new XmpIimTwins();
            if (this.ipmdRef[icc.itgIpmdTop][refIpmdId]){
                if (this.ipmdRef[icc.itgIpmdTop][refIpmdId][icc.itgXmpid]) shouldValFmts.xmp = true;
            }
            if (this.ipmdRef[icc.itgIpmdTop][refIpmdId]){
                if (this.ipmdRef[icc.itgIpmdTop][refIpmdId][icc.itgIimid]) shouldValFmts.iim = true;
            }

            let propTestFsd: MdStruct = testDataValueFsd.getFsData(refIpmdId);
            ...
            let propTestValue = propTestFsd[icc.fsdResValue]
            // **** test all possible data-properties of this reference-IPTC property
            let propRefFsd: MdStruct = refDataValueFsd.getFsData(refIpmdId);
            ...
            let xmpCrRow = new CompareResultRow(); // create empty row, will be filled if used
            let iimCrRow = new CompareResultRow(); // create empty row, will be filled if used
            let propRefValue = propRefFsd[icc.fsdResValue];
            // ** compare XMP
            if (icc.stateVxmp in propRefValue){
                let propRefValueXmp = propRefValue[icc.stateVxmp];
                let propTestValueXmp = propTestValue[icc.stateVxmp];
                if (typeof propRefValueXmp === 'string' || typeof propRefValueXmp === 'number'){
                    // this is a single plain value
                    if (icc.stateVxmp in propTestValue) {
                        if (propRefValueXmp !== propTestValueXmp) {
                            ...
                            xmpCrRow.comparedValueFormat = icc.stateVxmp;
                            xmpCrRow.comparedNamePath =
                                this._ipmdIdPath2nameSeq(xmpCrRow.comparedIpmdIdPath
                                + this.fsdLsep + icc.stateVxmp);
                        ...
                        xmpCrRow.comparedIpmdIdPath = refIpmdId;  // + this.fsdLsep + 'XMP';
                        xmpCrRow.comparedValueFormat = icc.stateVxmp;
                        xmpCrRow.comparedNamePath = this._ipmdIdPath2nameSeq(xmpCrRow.comparedIpmdIdPath
                            + this.fsdLsep + icc.stateVxmp)
                        xmpCrRow.refValue = propRefValueXmp.toString();
                        compareResultRows.push(xmpCrRow);
                    }
            ...
            // ** compare IIM
            if (icc.stateViim in propRefValue){
                let propRefValueIim = propRefValue[icc.stateViim];
                let propTestValueIim = propTestValue[icc.stateViim];
                if (propTestValueIim === undefined){ // no test value exists
                    if (shouldValFmts.iim){
                        ...
                        iimCrRow.comparedValueFormat = icc.stateViim;
                        iimCrRow.comparedNamePath = this._ipmdIdPath2nameSeq(iimCrRow.comparedIpmdIdPath
                            + this.fsdLsep + icc.stateViim);
                        iimCrRow.refValue = propRefValueIim.toString();
                    }
                }
                else {
                    if (typeof propRefValueIim === 'string' || typeof propRefValueIim === 'number') {
                        // this is a single plain value
                        if (icc.stateViim in propTestValue) {
                 ...
                 */
                if (iimCrRow.result !== '') {  // a .result MUST be set in a used IIM row --> push it, or merge it
                    if (compareOptions.mergeXmpIimRows) {  // option mergeXmpIimRows is active
                        if (xmpCrRow.result !== '') { // a .result MUST be set in a used XMP row
                            if (xmpCrRow.comparedIpmdIdPath === iimCrRow.comparedIpmdIdPath) {
                                ...
                                if (xmpCrRow.result === iimCrRow.result) {
                                    ...
                                    xmpCrRow.message = xmpCrRow.message.replace('XMP',
                                        'XMP and IIM');
                                    xmpCrRow.comparedNamePath = xmpCrRow.comparedNamePath + '+IIM';
                                    if (xmpCrRow.testValue != iimCrRow.testValue) {
                                        xmpCrRow.testValue = 'XMP: ' + xmpCrRow.testValue
                                            + ' | IIM: ' + iimCrRow.testValue;
                                    }
                                }
                        ...
                        } else { // IIM result exists, but not an XMP result
                ...
                else { // no used IIM row exists
                    if (xmpCrRow.result !== '') { // check if a used XMP row exists = a .result MUST be set
                        if (shouldValFmts.iim) {  // check if an IIM row should exist too
                            // Modify the XMP row
            ...
            if (icc.stateStruct in propRefValue) {
                let compareResultStructRows: CompareResultRow[] =
                    this._compareIpmdCheckerResultsStruct1(refDataValueFsd, testDataValueFsd,
                        refIpmdId + this.fsdLsep + icc.stateStruct, compareOptions);
    ...
    private _compareIpmdCheckerResultsStruct1(
        ...
        let fsdResult: MdStruct = refDataValueFsd.getFsData(thisImpdIdPath)
        if (fsdResult[icc.fsdResState] !== icc.fsdStFound)
            return [];
        let refDataValue: object = fsdResult[icc.fsdResValue];
        let compareResultRows: CompareResultRow[] = []; // this will be retured as result of this method

        if (Array.isArray(refDataValue)){ // multiple occurrences of the structure value in an array
            for(let idx = 0; idx < refDataValue.length; idx++){
                let thisThisImpdIdPath = thisImpdIdPath + this.fsdIsel + idx.toString();
                let singleCompareResultRows = this._compareIpmdCheckerResultsStruct2(refDataValueFsd,
                    testDataValueFsd, thisThisImpdIdPath, compareOptions);
                Array.prototype.push.apply(compareResultRows, singleCompareResultRows);
            }
        }
        else { // a single occurence of the structure value
            let singleCompareResultRows = this._compareIpmdCheckerResultsStruct2(refDataValueFsd,
                testDataValueFsd, thisImpdIdPath, compareOptions);
            Array.prototype.push.apply(compareResultRows, singleCompareResultRows);
        }
        return  compareResultRows;
    }
    ...
    private _compareIpmdCheckerResultsStruct2(
        ...
        let refValueIpmdIds: string[] = Object.keys(refDataValue);

        let compareResultRows: CompareResultRow[] = []; // this will be retured as result of this method
        for (let idx = 0; idx < refValueIpmdIds.length; idx++){
            let refIpmdId: string = refValueIpmdIds[idx];
            let refIpmdIdPath: string = thisImpdIdPath + this.fsdLsep + refIpmdId;
            let propTestFsd: MdStruct = testDataValueFsd.getFsData(refIpmdIdPath);
            if (propTestFsd[icc.fsdResState] === icc.fsdStErr){
                let crRow = new CompareResultRow();
                crRow.message = 'Test image: MISSING property';
            ...
            let propRefFsd: MdStruct = refDataValueFsd.getFsData(refIpmdIdPath);
            if (propRefFsd[icc.fsdResState] !== icc.fsdStFound){
                ...
                if (typeof propRefValueXmp === 'string' || typeof propRefValueXmp === 'number'){
                    ...
                    if (icc.stateVxmp in propTestValue) {
                        if (propRefValueXmp !== propTestValueXmp) {
                            ...
                            crRow.comparedValueFormat = icc.stateVxmp;
                            crRow.comparedNamePath = this._ipmdIdPath2nameSeq(crRow.comparedIpmdIdPath
                                + this.fsdLsep + icc.stateVxmp);
    ...
    private _checkReadyToCheck(){
        this._readyToCheck = !util1.objectIsEmpty(this.ipmdRef) && !util1.objectIsEmpty(this._ipmdStateDataTempl)
            && !util1.objectIsEmpty(this._ipmdStateData) && util1.objectIsEmpty(this._ipmdValueData);
    }
    ...
    private _ipmdIdPath2nameSeq(ipmdIdPath: string): string{
        const embdFmtIds: string[] = [icc.stateVxmp, icc.stateViim, icc.stateVexif];
        let nameSeq: string = '';

        let parentStructId: string = '';
        let impdIdPathParts: string[] = ipmdIdPath.split(this.fsdLsep);
        for (let idx = 0; idx < impdIdPathParts.length; idx++){
            ...
            if (impdIdPathParts[idx].startsWith('struct')){
                let structParts: string[] = impdIdPathParts[idx].split(this.fsdIsel);
                parentStructId = this.ipmdRef[icc.itgIpmdTop][impdIdPathParts[idx - 1]][icc.itgDataformat];
                if (structParts.length > 1){
                    let propIdx = +structParts[1]  + 1;
                    nameSeq += '[' + propIdx + ']'
                }
            }
            else { // id addresses a property
                let propName: string = '';
                if (parentStructId === '') { // ... at the top level
                    propName = this.ipmdRef[icc.itgIpmdTop][impdIdPathParts[idx]][icc.itgName];
                }
                else {  // ... inside a structure
                    propName = this.ipmdRef[icc.itgIpmdStruct][parentStructId][impdIdPathParts[idx]][icc.itgName];
                }
                if (idx > 0) nameSeq += '--' ;
                nameSeq += propName;
            }
    ...
    private _buildAltLangValue(etJsonData: MdStruct, basicPropId: string): string {
        let altLangStr: string = '';
        if (basicPropId in etJsonData) {
            altLangStr = etJsonData[basicPropId];
        }
        else return '';
        let etKeys = Object.keys(etJsonData);
        etKeys.forEach(etKey => {
            if (etKey.startsWith(basicPropId + '-')){
                let lang = etKey.substring(basicPropId.length + 1);
