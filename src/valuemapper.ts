 ...
 * @param ipmdValue
 */
export function ipmdToEt(ipmdValue: string): string {
    let etValue: string = '';
    if (ipmdValue === undefined) return etValue;
    if (ipmdValue === '') return etValue;
    switch (ipmdValue) {
        // telephone types
 ...
 */
export function etToIpmd(etValue: string): string {
    let ipmdValue: string = '';
    switch (ipmdValue) {
